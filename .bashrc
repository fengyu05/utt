...
##########################################################################################
if [ -d ~/bin ]; then
PATH=$PATH:~/bin:/opt/likewise/bin
fi

export EDITOR='vi'
export localhost=127.0.0.1
export HOME_IP=172.21.106.42
export PLAYGROUND=$HOME/workspace/playground
...
function source_file() {
  find -type d \( -path "*/.svn" -o -path "*/build" -o -path "*/.git" \) -prune -o -type f -exec grep -Il . {} \;
}
...
function svn_review_changelist() {
  svn review --changelist jython_udf_refactor
}
...
function vld_console_test() {
  ~/voldemort/bin/voldemort-shell.sh voldtest-hhw tcp://eat1-voldemort-nemo-vip.corp.linkedin.com:10103/ 
}
