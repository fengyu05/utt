...
def sendReportMail(options, args):
  ...
  receivers = [ x.endswith(LINKEDIN_MAIL_POSTFIX) and x or x + LINKEDIN_MAIL_POSTFIX   for x in receivers]
...
def loadData(options, args):
  cmd = 'rm -rf %s ; copy_from_gateway.py %s -o %s' % (options.temp, args[0], options.temp)
  runCommand(cmd)
