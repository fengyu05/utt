...
def toReadableTime(millis):
    if millis == -1:
        ...
        return datetime.fromtimestamp(millis / 1000.0).strftime(
            "%a, %d %b %Y %H:%M:%S +0000"
        )
...
def authenticate():
    ...
    cmd = "curl -k -X POST %s %s" % (
        makeDataSection(dataDict),
        getHandler(HANDLER_MANAGER),
    )
...
def authenticateForAction():
    ...
    if (not os.path.exists(sidFile)) or (
        time.time() - os.path.getmtime(sidFile) > options.expire
    ):
...
def createProject(name, description):
    ...
    cmd = "curl -k -X POST %s %s?action=create" % (
        makeDataSection(dataDict),
        getHandler(HANDLER_MANAGER),
    )
    result = teeRun(cmd)
...
def uploadProject(project, zipFile):
    ...
    cmd = 'curl -k -i -H "Content-Type: multipart/mixed" -X POST %s %s' % (
        makeDataSection(dataDict, True),
        getHandler(HANDLER_MANAGER),
    )
...
def fetchRunningExecutions(project, flow):
    authenticateForAction()
    sid = readSid()
    cmd = 'curl -k --get --data "session.id=%s&project=%s&flow=%s&ajax=%s" %s' % (
        sid,
        project,
        flow,
        "getRunning",
        getHandler(HANDLER_EXECUTOR),
    )
...
def runLastRunOfStatus(project, flow, status, fetchCount=20):
    ...
    ids = [
        x["id"]
        for x in jobsResult["nodes"]
        if x["status"] == "SUCCEEDED" or x["status"] == "SKIPPED"
    ]
    return runJobWithDisabledTask(project, flow, ids)
...
def main():
    ...
    parser.add_argument(
        "--host", dest="host", default=None, help="Azkaban host address"
    )
    parser.add_argument(
        "--config", dest="config", default=DEFAULT_CONFIG, help="config file"
    )
    parser.add_argument(
        "-e", "--env", dest="env", default=ENV_HOLDEM, help="Azkaban env"
    )
    parser.add_argument(
        "-p", "--params", dest="params", type=str, nargs="*", help="Job params"
    )
    parser.add_argument(
        "--expire",
        dest="expire",
        ...
        help="Session expire time in second",
    )
    parser.add_argument(
        "--concurrentOption",
        dest="concurrentOption",
        default="",
        help="concurrentOption: [ingore | pipeline | queue]",
    )
    parser.add_argument(
        "--mute", dest="mute", default=False, help="mute stdout except return text"
    )
