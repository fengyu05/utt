...
def main():
    ...
    parser = OptionParser()
    parser.add_option(
        "-r", "--reciever", dest="reciever", default="zhdeng@linkedin.com", help=""
    )
    parser.add_option(
        "-s",
        "--sender",
        dest="sender",
        default="hd-charter-noreply@linkedin.com",
        help="",
    )
    ...
    if options.file:
        sendReportMail(
            options.sender, [options.reciever], options.title, options.file.split(",")
        )
    else:
